...
namespace NativeUI
{
    public class BigMessageHandler
    {
        ...
        public async void ShowWeaponPurchasedMessage(string bigMessage, string weaponName, uint weapon, int time = 5000)
        {
            await Load();
            _start = Game.GameTime;
            _sc.CallFunction("SHOW_WEAPON_PURCHASED", bigMessage, weaponName, unchecked((int)weapon), "", 100);
        ...
        public async void ShowMpWastedMessage(string msg, string sub, int time = 5000)
        {
            await Load();
            _start = Game.GameTime;
            _sc.CallFunction("SHOW_SHARD_WASTED_MP_MESSAGE", msg, sub, 5);
            _sc.CallFunction("TRANSITION_IN");
            _timer = time;
        }
